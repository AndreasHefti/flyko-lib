...
object GeomUtils {
    ...
    fun magnitude(v: Vector2i) = sqrt((v.v0 * v.v0 + v.v1 * v.v1).toFloat())
    fun magnitude(v: Vector4i) = sqrt((v.v0 * v.v0 + v.v1 * v.v1 + v.v2 * v.v2 + v.v3 * v.v3).toFloat())
    fun magnitude(v: Vector2f) = sqrt(v.v0 * v.v0 + v.v1 * v.v1)
     ...
     */
    fun intersectCircle(circle1: Vector3i, circle2: Vector3i): Boolean {
     ...
     */
    fun intersectCircle(circle1: Vector3f, circle2: Vector3f): Boolean {
        val dx = (circle1.x + circle1.radius) - (circle2.x + circle2.radius)
        val dy = (circle1.y + circle1.radius) - (circle2.y + circle2.radius)
        return sqrt((dx * dx + dy * dy).toDouble()) < circle1.radius + circle2.radius
    }
     ...
     */
    fun intersectCircle(circle: Vector3i, rect: Vector4i): Boolean {
     ...
     *  Vector4i : (x,y,width, height)
     */
    fun intersectCircle(circle: Vector3f, rect: Vector4f): Boolean {
        val dx = abs(circle.x - rect.x) - rect.width / 2
        val dy = abs(circle.y - rect.y) - rect.height / 2
    ...
    fun intoBoundary(v: Int, lower: Int, upper: Int): Int =
        when {
            (v < lower) -> lower
            (v > upper) -> upper
            else -> v
    ...
    fun intoBoundary(v: Float, lower: Float, upper: Float): Float =
        when {
            (v < lower) -> lower
            (v > upper) -> upper
            else -> v
    ...
    fun angleX(v: Vector2f) = atan2(v.y, v.x)
    fun angleY(v: Vector2f) = atan2(v.x, v.y)
    fun radToDeg(r: Float, invert: Boolean = false) =
        if (invert)
            -r * (180f / PI_F)
        else
            r * (180f / PI_F)
     ...
     */
    fun transformRange(n: Float, r0: Float = 0f, r1: Float = 1f, v0: Float = 0f, v1: Float = 1f): Float =
        v0 + ((v1 - v0) / (r1 - r0)) * (n - r0)
