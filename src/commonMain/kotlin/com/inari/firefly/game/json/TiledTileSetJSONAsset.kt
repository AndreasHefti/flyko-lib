...
class TiledTileSetJSONAsset private constructor() : Asset() {
    ...
    private var tileSetKey = NO_COMPONENT_KEY
    private var textureAssetKey = NO_COMPONENT_KEY
    ...
    override fun assetIndex(index: Int): Int = tileSetKey.instanceIndex
    ...
    override fun load() {
        ...
        if (Asset.exists(atlasResource)) {
            val texKey = Asset.getKey(atlasResource)
            if (texKey.type.aspectIndex == Texture.aspectIndex) {
                textureAssetKey = texKey
            }
        }
        if (textureAssetKey == NO_COMPONENT_KEY) {
            textureAssetKey = Texture.buildActive {
        ...
        // create tile set
        tileSetKey = TileSet.build {
            name = tileSetJson.name
            textureRef(this@TiledTileSetJSONAsset.textureAssetKey)
    ...
    override fun dispose() {
        Asset.delete(tileSetKey)
        tileSetKey = NO_COMPONENT_KEY
        textureAssetKey = NO_COMPONENT_KEY
    }

    companion object :  ComponentSubTypeSystem<Asset, TiledTileSetJSONAsset>(Asset, "TiledTileSetJSONAsset") {
        override fun create() = TiledTileSetJSONAsset()
    }
